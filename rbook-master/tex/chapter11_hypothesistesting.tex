...
At this point some of you might be wondering if this is a ``real'' hypothesis test, or just a toy example that I made up. It's real. In the previous discussion I built the test from first principles, thinking that it was the simplest possible problem that you might ever encounter in real life. However, this test already exists: it's called the {\it binomial test}, and it's implemented by an \R\ function called \rtext{binom.test()}. To test the null hypothesis that the response probability is one-half \rtext{p = .5},\FOOTNOTE{Note that the \rtextsmall{p} here has nothing to do with a $p$ value. The \rtextsmall{p} argument in the \rtextsmall{binom.test()} function corresponds to the probability of making a correct response, according to the null hypothesis. In other words, it's the $\theta$ value.} using data in which \rtext{x = 62} of \rtext{n = 100} people made the correct response, here's how to do it in \R:

\begin{rblock1}
> @usr{binom.test( x=62, n=100, p=.5 )}
