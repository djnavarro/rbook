...
```{r packageinstall, fig.cap=c("The package installation dialog box in Rstudio (panel a). ", "When you start typing, you'll see a dropdown menu suggest a list of possible packages that you might want to install (panel b)"), echo=FALSE}
...
```{r updatepackages, packagepanel, fig.cap="The Rstudio dialog box for updating packages", echo=FALSE}
...
- `car`. This is the *Companion to Applied Regression* package, which accompanies the excellent book of the same name by \citeA{Fox2011}. It provides a lot of very powerful tools, only some of which we'll touch in this book.
...
```{r workspace2, fig.cap="The Rstudio \"Environment\" panel shows you the contents of the workspace. Compare this \"grid\" view to the \"list\" view in Figure \\@ref(fig:workspace)", echo=FALSE}
...
knitr::kable(rbind(
                  ...
                  c("C:\\\\Users\\\\dan\\\\nerdstuff ", "..\\\\nerdstuff")),
caption = 'Basic arithmetic operations in R. These five operators are used very frequently throughout the text, so it\'s important to be familiar with them at the outset. There are others as well, which I\'ll discuss in Chapter \\@ref(datahandling)',col.names = c("absolute path (i.e., from root)"  , "relative path (i.e. from C:\\Users\\danRbook)"),
  booktabs = TRUE)
```
...
knitr::include_graphics("./rbook-master/img/introR/booksalescsv.png")
