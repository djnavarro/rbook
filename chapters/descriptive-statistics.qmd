...
d.cor <- 0.0
out.0 <- as.data.frame(MASS::mvrnorm(100, mu = c(0,0), 
...
d.cor <- 0.33
out.1 <- as.data.frame(MASS::mvrnorm(100, mu = c(0,0), 
                             Sigma = matrix(c(1,d.cor,d.cor,1), ncol = 2), 
                             empirical = TRUE))
plot(out.1,frame.plot=FALSE, axes=FALSE,xlab=paste("r =",d.cor),ylab="")

d.cor <- 0.66
out.2 <- as.data.frame(MASS::mvrnorm(100, mu = c(0,0), 
                             Sigma = matrix(c(1,d.cor,d.cor,1), ncol = 2), 
                             empirical = TRUE))
...
Calculating correlations in R can be done using the `cor()` command. The simplest way to use the command is to specify two input arguments `x` and `y`, each one corresponding to one of the variables. The following extract illustrates the basic usage of the function:^[If you are reading this after having already completed Chapter \@ref(hypothesistesting) you might be wondering about hypothesis tests for correlations. R has a function called `cor.test()` that runs a hypothesis test for a single correlation: hypothesis tests for correlations are discussed in more detail in Section \@ref(corrhyp).]

```{r}
cor(southern_slam$home_score, southern_slam$visitor_score)
```
...
#| echo: false
#| fig-height: 8
op <- par(mfcol = c(2, 2)) # Create a 2 x 2 plotting matrix
