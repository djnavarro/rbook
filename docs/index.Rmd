...
Thanks to the incredible efforts of [Emily Kothe](https://twitter.com/emilyandthelime), there is a tentative bookdown adaptation (version 0.6.1) of the original LSR book. A little care is probably required because the translation from TeX to bookdown isn't perfect and there may be errors:

- [LSR version 0.6.1](./book/) (bookdown)

The book is associated with the `lsr` package on CRAN and GitHub. The package is probably okay for many introductory teaching purposes, but some care is required. The package does have some limitations (e.g., the `etaSquared` function does strange things for unbalanced ANOVA designs), and I haven't updated it in a while. 

There are a few projectsI'm aware of that adapt or extend content from LSR
...
- I made a first attempt at an adaptation, [R for Psychological Science](https://psyr.org/), though that has stalled for the moment

Oh, also, I've started work on a Tidyverse-based update to *Learning Statistics with R*...

- The work in progress book: https://djnavarro.github.io/tidylsrbook/
- The associated package: https://github.com/djnavarro/tidylsr
